...
/// PMM begin
double Godunov2D(double a, double b) {
    double diff = fabs(a - b);
    if (diff >= 1.0 / F)
        return fmin(a, b) + 1.0 / F;
    else
        return (a + b + sqrt(2.0 / (F * F) - diff * diff)) / 2.0;
}
void iSubSweep(Subdomain* sd, int dir) {
    int start = (dir == 1) ? 1 : BLOCK_SIZE;
    int end   = (dir == 1) ? BLOCK_SIZE + 1 : 0;

    for (int i = start; i != end; i += dir) {
        for (int j = 1; j <= BLOCK_SIZE; j++) {
            double tx1 = sd->data[i - 1][j]; // left
            double tx2 = sd->data[i + 1][j]; // right
            double ty1 = sd->data[i][j - 1]; // up
            double ty2 = sd->data[i][j + 1]; // down

            double t_new = Godunov2D(fmin(tx1, tx2), fmin(ty1, ty2));

            if (t_new < sd->data[i][j])
                sd->data[i][j] = t_new;
        }
    }
}

void jSubSweep(Subdomain* sd, int dir) {
    int start = (dir == 1) ? 1 : BLOCK_SIZE;
    int end   = (dir == 1) ? BLOCK_SIZE + 1 : 0;
